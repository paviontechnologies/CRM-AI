    ...
    <>
      ...
      {!open && (
        <button
          ...
          className="fixed bottom-6 right-6 z-50 flex items-center gap-2 pl-4 pr-5 py-3 bg-accent text-onaccent rounded-full hover:shadow-xl hover:scale-105 transition-all"
        >
      ...
      {/* Panel */}
      {open && (
        <div className="fixed bottom-6 right-6 z-50 w-[calc(100vw-3rem)] sm:w-[400px] h-[600px] max-h-[calc(100vh-3rem)] bg-surface rounded-2xl shadow-2xl border border-line flex flex-col overflow-hidden">
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 bg-accent text-onaccent flex-shrink-0">
            ...
            <button onClick={() => setOpen(false)} className="text-brand-100 hover:text-onaccent transition-colors">
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                <div className="space-y-2">
                  {SUGGESTIONS.map((s) => (
                    <button
                      key={s}
                      onClick={() => send(s)}
                      className="w-full text-left px-3 py-2.5 bg-subtle hover:bg-brand-50 hover:text-brand-700 rounded-xl text-sm text-muted transition-colors"
                    >
                      {s}
                    </button>
                  ))}
                ...
                <div className={`max-w-[85%] ${m.role === 'user' ? 'order-2' : ''}`}>
                  <div
                    className={`px-3.5 py-2.5 rounded-2xl text-sm whitespace-pre-wrap ${
                      m.role === 'user'
                        ? 'bg-brand-600 text-onaccent rounded-br-sm'
                        : m.error
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="bg-subtle rounded-2xl rounded-bl-sm px-4 py-3 flex items-center gap-2">
                  <Loader2 className="w-4 h-4 text-faint animate-spin" />
                  <span className="text-xs text-faint">Working on it…</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="p-3 border-t border-line flex-shrink-0">
            <div className="flex items-end gap-2 bg-subtle rounded-2xl px-3 py-2 focus-within:ring-2 focus-within:ring-brand-500">
              <textarea
                ref={inputRef}
                value={input}
                ...
                className="flex-1 bg-transparent text-sm text-ink focus:outline-none resize-none max-h-24 py-1"
              />
              <button
                onClick={() => send(input)}
                disabled={!input.trim() || loading}
                className="flex-shrink-0 w-8 h-8 bg-brand-600 hover:bg-brand-700 disabled:opacity-40 text-onaccent rounded-xl flex items-center justify-center transition-colors"
              >
                <Send className="w-4 h-4" />
              </button>
            </div>
            <p className="text-[10px] text-faint text-center mt-1.5">
