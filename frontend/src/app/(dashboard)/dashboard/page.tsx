    ...
    <div className="px-5 py-3.5 hover:bg-slate-50 transition-colors">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 min-w-0 flex-1">
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-white text-xs font-semibold flex-shrink-0 mt-0.5 ${accent}`}>
            {lead.companyName?.charAt(0)?.toUpperCase() ?? 'L'}
          </div>
          <div className="min-w-0">
            <Link href={`/leads/${lead.id}`} className="text-sm font-semibold text-slate-900 hover:text-brand-600 truncate block">
              {lead.companyName}
            </Link>
    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-slate-950 p-6">
        ...
        <div className="relative flex items-start justify-between gap-6 flex-wrap">
          <div className="flex-1 min-w-0">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/10 text-brand-200 text-xs font-medium mb-3">
              <Sparkles className="w-3.5 h-3.5" /> AI Lead Finder
            </div>
            <h2 className="text-white text-lg font-semibold">Generate real buyer leads in one click</h2>
            <p className="text-slate-400 text-sm mt-1 mb-4 max-w-lg">
              Pick a market and the AI finds matching companies with contacts — deduplicated against your existing leads.
            </p>

            {generateMsg && (
              <div className="inline-flex items-center gap-2 bg-white/10 rounded-lg px-3 py-1.5 text-sm text-white font-medium mb-3">
                {generating && <div className="w-3.5 h-3.5 border-2 border-white/60 border-t-transparent rounded-full animate-spin" />}
                {generateMsg}
              </div>
            )}
              ...
              {QUICK_GENERATE_CONFIGS.map((cfg, idx) => (
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors disabled:opacity-60 disabled:cursor-not-allowed ${
                    generating && activeConfig === idx ? 'bg-white text-brand-700' : 'bg-white/10 hover:bg-white/20 text-slate-200'
                  }`}
                >
          ...
          <div className="hidden md:flex flex-col items-center justify-center bg-white/[0.07] border border-white/10 rounded-xl px-5 py-4 min-w-[130px] text-center">
            <div className="text-3xl font-bold text-white tabular-nums">{stats?.totalLeads ?? 0}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="overflow-hidden">
          ...
          <div className="divide-y divide-slate-100">
            {hotLeads.length === 0
              ? <div className="px-6 py-10 text-center text-slate-400 text-sm">No hot leads yet — use AI Lead Finder above.</div>
              : hotLeads.map((lead) => (
                  <LeadRow key={lead.id} lead={lead} accent="bg-gradient-to-br from-brand-500 to-brand-700"
                    meta={lead.notes ? <p className="text-xs text-slate-400 mt-1 line-clamp-1">{lead.notes}</p> : null} />
                ))}
          </div>
        </Card>
        ...
        <Card className="overflow-hidden">
          ...
          <div className="divide-y divide-slate-100">
            {recentLeads.length === 0
              ? <div className="px-6 py-10 text-center text-slate-400 text-sm">No leads yet — generate some above.</div>
              : recentLeads.map((lead) => (
                  <LeadRow key={lead.id} lead={lead} accent="bg-gradient-to-br from-slate-600 to-slate-800"
                    meta={lead.tags ? (
                      <div className="flex items-center gap-1 mt-1 flex-wrap">
                        <Tag className="w-3 h-3 text-slate-300" />
                        {lead.tags.split(',').slice(0, 3).map((tag: string) => (
                          <span key={tag} className="text-xs bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded">{tag.trim()}</span>
                        ))}
                      </div>
                    ) : null} />
                ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2 p-6">
          <h3 className="font-semibold text-slate-900 mb-5">Leads this week</h3>
        ...
        <Card className="p-6">
          ...
          ) : <div className="flex items-center justify-center h-48 text-slate-400 text-sm">No source data yet</div>}
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="p-6">
          <h3 className="font-semibold text-slate-900 mb-5">Pipeline funnel</h3>
          {statusData.length > 0 ? (
          ...
          ) : <div className="flex items-center justify-center h-48 text-slate-400 text-sm">No pipeline data yet</div>}
        </Card>
            ...
            {activities.slice(0, 7).map((a: any) => (
              <div key={a.id} className="flex items-start gap-3 py-2">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm text-slate-800">{a.notes || a.type?.replace(/_/g, ' ')}</p>
